...
const state = { school: "havo", profile: "cm", choices: {}, freeChoice: "", extraEnabled: false, extraChoice: "", resultRequested: false };
...
function availableExtraChoices() {
  return (currentProfile().optionalExtra || []).filter((choice) => !selectedChoiceNames().includes(choice) && choice !== state.freeChoice);
}
...
function renderChoices() {
  ...
  if (extraChoices.length) {
    ...
    toggle.querySelector("input").addEventListener("change", (event) => { state.extraEnabled = event.target.checked; renderSubjects(); });
    ...
    extraChoices.forEach((choice) => {
      ...
      label.querySelector("input").addEventListener("change", () => { state.extraChoice = choice; state.extraEnabled = true; renderChoices(); renderSubjects(); });
...
function subject(name, kind) {
  return { name, kind, core: name === "Nederlands" || name === "Engels" || mathSubjects.has(name), hasCe: !noCeSubjects.has(name) };
}
...
function fmt(value) { return Number.isFinite(value) ? value.toLocaleString("nl-NL", { minimumFractionDigits: 1, maximumFractionDigits: 1 }) : "-"; }
...
function calculate() {
  ...
  if (!isComplete) {
    ...
    rule("Het CE-gemiddelde moet minimaal 5,5 zijn.", "pending");
  ...
  rule(`CE-gemiddelde is ${fmt(ceAvg)} en moet minimaal 5,5 zijn.`, ceRule ? "pass" : "fail");
  ...
  rule(`Combinatiecijfer telt mee voor compensatie. Totaal compensatiepunten: ${compensation}.`, "pass");
  ...
  if (passed) {
    ...
    setDetails([`Je CE-gemiddelde is ${fmt(ceAvg)}.`, "Je voldoet aan alle eisen."], "Alles klopt");
  } else {
    setStatus("danger", "Gezakt", "Je bent niet geslaagd", failed[0]);
    setDetails([`Je CE-gemiddelde is ${fmt(ceAvg)}.`, ...failed], "Dit moet je nog halen");
  }
}
...
function failedMessages(result) {
  const messages = [];
  if (!result.ceRule) messages.push(`Je CE-gemiddelde is ${fmt(result.ceAvg)}. Dat moet minimaal 5,5 zijn.`);
